...
param_dist = {
    'iterations': randint(50, 201),  # Number of boosting iterations
    'learning_rate': [0.01, 0.1, 0.2],  # Step size at each iteration
    'depth': randint(3, 8),  # Depth of the trees
}

# n_iter参数指定了随机搜索要执行的随机采样次数，可以根据需要设置为合适的值
random_search = RandomizedSearchCV(catboost, param_distributions=param_dist, n_iter=20, cv=5, random_state=1)

random_search.fit(X_train_std, y_train)

best_catboost_classifier = random_search.best_estimator_
best_catboost_classifier.fit(X_train_std, y_train)
...
save_path = "D:/Analysis of diabetes/ROC/" + "CatBoost_Random_Search_" + filename
